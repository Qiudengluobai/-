...
Diagnostics.testInjection = function() {
  ...
  try {
    ...
    return DataService.save(testMem).then(function(saved) {
      // 验证关键词已入索引，若未入则等待
      var kwOk = (LorebookManager._keywordIndex || {})['测试之剑'] || (LorebookManager._keywordIndex || {})['诊断注入测试'];
      results.push(kwOk ?
        _diagOk('✓ 关键词已入索引') :
        _diagFail('✗ 关键词未入索引（_keywordIndex 中无测试之剑/诊断注入测试）', 'DataService.save hook 中的 _addToIndex 可能未执行'));
      var delay = kwOk ? 100 : 500;
      return new Promise(function(resolve) {
        setTimeout(function() {
          // 二次检查
          var kwOk2 = (LorebookManager._keywordIndex || {})['测试之剑'] || (LorebookManager._keywordIndex || {})['诊断注入测试'];
          if (!kwOk2) {
            results.push(_diagFail('✗ 延迟后关键词仍未入索引', 'LorebookManager._addToIndex 未被调用'));
...
// 流⑩ CSS 可见性诊断 — 扫描所有 mm-* 元素，检测颜色/对比度/继承问题
Diagnostics.testCSS = function() {
  var results = [];
  try {
    var selectors = [
      '.mm-tab-btn', '.mm-tab-btn.active', '.mm-tab-icon', '.mm-tab-label',
      '.mm-btn', '.mm-btn-primary', '.mm-btn-sm', '.mm-btn-xs',
      '.mm-modal-header h3', '.mm-modal-close',
      '.mirror-modal', '.mm-modal',
      '.memory-card', '.mm-card-content',
      '.mm-auto-task-row', '.mm-auto-progress-bar',
      '.mm-modal-overlay',
      '.mm-floating-ball-new',
      '.mirror-memory-list', '.mirror-batch-bar',
      '.mirror-tag-cloud'
    ];
    ...
    for (var si = 0; si < selectors.length; si++) {
      ...
      try {
        var els = targetDoc.querySelectorAll(sel);
        if (!els || els.length === 0) continue;
        for (var ei = 0; ei < Math.min(els.length, 3); ei++) {
          try {
            var cs = targetWin.getComputedStyle(els[ei]);
            var color = cs.color || '';
            var bg = cs.backgroundColor || '';
            var opacity = cs.opacity || '1';
            var fz = cs.fontSize || '';

            // 检查颜色是否"正常"（非纯黑/非透明/非继承的奇怪颜色）
            var isTransparent = color === 'rgba(0, 0, 0, 0)' || opacity === '0';
            var isPureBlack = color === 'rgb(0, 0, 0)';
            var bgIsTransparent = bg === 'rgba(0, 0, 0, 0)' || bg === 'transparent';
            var fontSizeZero = fz === '0px';

            if (isTransparent) {
              issues.push({ sel: sel, issue: '文字透明/不可见', color: color, opacity: opacity });
              warned++;
            } else if (fontSizeZero) {
              issues.push({ sel: sel, issue: '字体大小为0', fz: fz });
              warned++;
            } else if (isPureBlack && bgIsTransparent) {
              // 黑字+透明背景可能是正常的（继承），只记录
              ok++;
            } else {
              ok++;
            }
          } catch(e2) { warned++; issues.push({ sel: sel, issue: '计算样式失败: ' + (e2.message || '') }); }
        }
      } catch(e) {}
    }

    // 汇总面板中实际可见元素的颜色
    results.push({ pass: true, msg: '扫描了 ' + selectors.length + ' 个选择器，发现 ' + warned + ' 个可疑项' });

    // 抽样对比：取面板中所有可见文本元素
    try {
      var allMm = targetDoc.querySelectorAll('[class*="mm-"], [class*="mirror-"]');
      var colorMap = {};
      var bgMap = {};
      for (var ai = 0; ai < Math.min(allMm.length, 50); ai++) {
        try {
          var cs2 = targetWin.getComputedStyle(allMm[ai]);
          var c2 = cs2.color || ''; var b2 = cs2.backgroundColor || '';
          if (!colorMap[c2]) colorMap[c2] = { count: 0, example: (allMm[ai].className || '').substring(0, 40) };
          colorMap[c2].count++;
          if (!bgMap[b2]) bgMap[b2] = { count: 0 };
          bgMap[b2].count++;
        } catch(e) {}
      }

      // 报告最常用的颜色值
      var colorKeys = Object.keys(colorMap).sort(function(a, b) { return colorMap[b].count - colorMap[a].count; });
      results.push({ pass: true, msg: '文字颜色分布(Top 5): ' +
        colorKeys.slice(0, 5).map(function(k) { return colorMap[k].count + '× ' + k; }).join(', ') });

      var bgKeys = Object.keys(bgMap).sort(function(a, b) { return bgMap[b].count - bgMap[a].count; });
      results.push({ pass: true, msg: '背景颜色分布(Top 5): ' +
        bgKeys.slice(0, 5).map(function(k) { return bgMap[k].count + '× ' + k; }).join(', ') });

      // 检测"浅色文字+浅色背景"的低对比度问题
      var lowContrast = 0;
      for (var li = 0; li < Math.min(allMm.length, 50); li++) {
        try {
          var cs3 = targetWin.getComputedStyle(allMm[li]);
          var isLightColor = cs3.color.match(/rgb\(2[3-5][0-9],\s*2[3-5][0-9]/); // 灰色系文字
          var isLightBg = cs3.backgroundColor.match(/rgb\(2[4-5][0-9],\s*2[4-5][0-9]/); // 白色系背景
          if (isLightColor && isLightBg && (allMm[li].textContent || '').length > 2) {
            lowContrast++;
            if (issues.length < 10) {
              issues.push({
                sel: (allMm[li].className || '').substring(0, 40),
                issue: '低对比度: ' + cs3.color + ' / ' + cs3.backgroundColor,
                text: (allMm[li].textContent || '').substring(0, 20)
              });
            }
          }
        } catch(e) {}
      ...
      if (lowContrast > 0) {
        results.push(_diagFail('发现 ' + lowContrast + ' 个低对比度元素（灰色文字+浅色背景）', '检查宿主页面 CSS 是否覆盖了面板颜色'));
      } else {
        results.push(_diagOk('未发现低对比度元素'));
      }
    } catch(e) {
      results.push({ pass: true, msg: '抽样分析异常: ' + (e.message || '') });
    }

    // 列出有问题的具体选择器
    for (var ii = 0; ii < Math.min(issues.length, 8); ii++) {
      results.push({ pass: false, msg: '⚠ ' + issues[ii].sel + ': ' + issues[ii].issue +
        (issues[ii].color ? ' (' + issues[ii].color + ')' : '') });
    }

    return Promise.resolve({ flow: 'CSS检测', pass: warned === 0, results: results });
  } catch(e) {
    results.push(_diagFail('CSS检测异常: ' + (e.message || '')));
    return Promise.resolve({ flow: 'CSS检测', pass: false, results: results });
  }
};
...
function _renderPanelContent() {
  ...
  function _switchTab(tabId) {
    [tabOverview, tabMemories, tabTimeline].forEach(function(t) { t.className = 'mm-tab-btn'; t.style.color = ''; });
...
UIManager._showAutoPanel = function() {
  ...
  function renderEasy() {
    ...
    infoDiv.innerHTML = '<b style="color:#555">💡 自动化规则说明</b><br>' +
      '<b style="color:#2c2c2c">关</b> = 不使用 | <b style="color:#2c2c2c">偶尔</b> = 约20轮对话触发一次 | <b style="color:#2c2c2c">频繁</b> = 约5轮对话触发一次<br>' +
      '进度条表示距离下次触发的完成度（<b style="color:#2c2c2c">N/20</b> = 已过N轮，还需20-N轮）<br>' +
      '触发后的内容会<u style="color:#2c2c2c">按下方选择的输出方式</u>呈现给你';
...
UIManager._showDiagnostics = function() {
  ...
  function renderReport(report) {
    ...
    var flowNames = { '采集流': '📥 采集流', '注入流': '💉 注入流', '规则流': '⚙ 规则流', '生命流': '🔄 生命流', '存储流': '💾 存储流', '输入框': '⌨ 输入框', '面板': '🖥 面板', '完整性': '🔍 完整性', 'CSS检测': '🎨 CSS检测', '复制流': '📋 复制流' };
    ...
    for (var ti = 0; ti < tests.length; ti++) {
      ...
      html += '<div style="padding:6px 10px;background:' + (t.pass ? '#e8f5e9' : '#ffebee') + ';font-size:13px;font-weight:500;color:#333!important">' + icon + ' ' + (flowNames[t.flow] || t.flow) + '</div>';
...
function _injectCSS() {
  ...
  style.textContent = [
    ...
    /* 按钮基础 */
    '.mm-btn{font-size:11px!important;padding:4px 10px;border:1px solid var(--mm-border);border-radius:16px;cursor:pointer;background:#fff!important;color:var(--mm-brown)!important;transition:all 0.15s;white-space:nowrap;flex-shrink:0;font-family:inherit}',
    '.mm-btn:hover{background:var(--mm-pink-light)!important;border-color:#d4b0b0}',
    '.mm-btn:active{background:#f0e0e0!important}',
    '.mm-btn-primary{background:var(--mm-pink)!important;color:#fff!important;border-color:var(--mm-pink)}',
    '.mm-btn-primary:hover{background:var(--mm-pink-dark)!important}',
    ...
    /* 标签导航 */
    '.mm-tab-btn{padding:7px 4px;font-size:12px!important;color:#2c2c2c!important;background:none!important;border:none;border-bottom:2px solid transparent;cursor:pointer;font-family:inherit}',
    '.mm-tab-btn.active{border-bottom-color:var(--mm-pink)!important;color:var(--mm-pink)!important;font-weight:600!important}',
    ...
    '.mm-modal,.mirror-modal{background:#fff;border:1px solid var(--mm-border);border-radius:12px;padding:22px 28px;max-width:500px;width:92%;max-height:85vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,0.1);color:#2c2c2c}',
    '.mm-modal *,.mirror-modal *,.wb-body *{color:inherit;font-family:inherit}',
    ...
    '.mm-modal-header h3{font-size:15px;color:#2c2c2c!important;margin:0}',
    ...
    '.wb-body{container-type:inline-size;container-name:mirror-panel;color:#2c2c2c;background:#fff}',
    /* 工具栏响应式 */
    ...
    '@media (max-width:768px){',
      ...
      '.mm-tab-btn{padding:6px 4px;font-size:11px;color:#2c2c2c!important}',
